  ...
  return <main className="terminal-shell">
    ...
    <section className="scanner-layout" id="scanner">
      <div className="map-panel">
        ...
        <div className="mobile-ranks" aria-label="Ranked opportunities">{filtered.map((item) => <button key={item.symbol} className={selected.symbol === item.symbol ? "mobile-selected" : ""} onClick={() => setSelectedSymbol(item.symbol)}><AssetMark symbol={item.symbol} size={22} decorative /><span><strong>{item.symbol.replace("USDT", "")}</strong><small>{item.direction}</small></span><i className={`status-dot dot-${item.status}`} /><strong className={item.modeledNetBps > 0 ? "signal-positive" : "signal-negative"}>{formatBps(item.modeledNetBps)}</strong></button>)}</div>
      ...
      <aside className="evidence-panel" aria-live="polite">
        <div className="evidence-head"><div><p className="micro-label">SELECTED VERDICT</p><h2 className="selected-pair-title"><AssetMark symbol={selected.symbol} size={28} decorative />{selected.symbol}</h2></div><span className={`verdict-chip chip-${selected.status}`}>{verdictLabel(selected.status)}</span></div>
    ...
    <section className="ledger-panel" aria-labelledby="ledger-heading"><div className="panel-topline"><div><p className="micro-label">ACCESSIBLE LEDGER</p><h2 id="ledger-heading">Every verdict, in plain rows.</h2></div><p>Keyboard-select any pair to inspect the same cost stack.</p></div><div className="ledger-scroll"><table><thead><tr><th scope="col">Pair</th><th scope="col">Route</th><th scope="col">Funding diff.</th><th scope="col">Costs</th><th scope="col">Modeled net</th><th scope="col">Freshness</th><th scope="col">Verdict</th></tr></thead><tbody>{opportunities.map((item) => <tr key={item.symbol} className={selected.symbol === item.symbol ? "ledger-selected" : ""}><td><button className="pair-button" onClick={() => setSelectedSymbol(item.symbol)}><AssetMark symbol={item.symbol} size={20} decorative />{item.symbol}</button></td><td>{item.direction}</td><td>{formatBps(item.grossFundingBps)}</td><td>−{item.totalCostBps.toFixed(2)} bp</td><td className={item.modeledNetBps > 0 ? "signal-positive" : "signal-negative"}>{formatBps(item.modeledNetBps)}</td><td>{Math.max(0, Math.round(item.freshnessMs / 1000))}s</td><td><span className={`ledger-status ledger-${item.status}`}>{verdictLabel(item.status)}</span></td></tr>)}{rows.filter((row) => !row.opportunity).map((row) => <tr key={row.symbol}><td>{row.symbol}</td><td colSpan={4}>{row.reason ?? "Waiting for validated public data."}</td><td>—</td><td><span className="ledger-status">Unavailable</span></td></tr>)}</tbody></table></div></section>
    <NewsBriefs />
